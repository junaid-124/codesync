...
// 🔑 IMPORT CHANGE: Add Link component for navigation
...
                // 🔑 FIX LOGIC: Find the corresponding roll number from TABLE_DATA
                const fullUserData = TABLE_DATA.find(t => t.name === user.name.toUpperCase());
                // Fallback: Use roll number if found, otherwise use a default ID (name)
                const rollNumber = fullUserData?.roll || user.name.toLowerCase().replace(/\s/g, '-');
                
                return (
                <Link 
                    key={user.rank} 
                    to={`/profile/${rollNumber}`} // Use the safely retrieved rollNumber
                    // 🔑 ADDED: Open in new tab
                    target="_blank"
                    className={`${styles.podiumCard} ${user.themeClass}`}
                  >
              
              {/* Floating Top Icon */}
...
                    {/* 🔑 KEY CHANGE: Wrap the profile name in a Link */}
                    <Link to={`/profile/${row.roll}`} className={styles.profileNameLink} target="_blank">
